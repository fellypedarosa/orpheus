...
*   **Contexto Compartilhado (Manifesto e Blueprints):** Se existir um arquivo `.orpheus_manifest.json` no diretório raiz, ou arquivos de documentação (`.md`, `.yaml`, `.json`) dentro da pasta `.orpheus/`, eles serão automaticamente injetados como contexto de restrição global ("Blueprints") na diretiva de todos os soldados despachados. *Nota: O seu agente/orquestrador (Antigravity) pode descobrir e usar isso criando a pasta `.orpheus/` e colocando os arquivos lá, pois o MCP lida com a injeção invisível.*
